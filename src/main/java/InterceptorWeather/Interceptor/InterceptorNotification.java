...
@Slf4j
public class InterceptorNotification implements Interceptor {
    @Override
    public boolean process(Context c) {
        log.warn("test if it needs warning...");
        var m = c.getMeasurement();
        if (m.getTemperature() >= DANGER_TEMPERATURE_UPPER
                || m.getTemperature() <= DANGER_TEMPERATURE_LOWER) {
            log.warn("Temperature is outside of the safe range: {}", m.getTemperature());
        }

        if (m.getPressure() >= DANGER_PRESSURE_UPPER
                || m.getPressure() <= DANGER_PRESSURE_LOWER) {
